...
class Experiment(object):
    ...
    def __init__(self, sandbox=True, keywords=None, lifetime=1209600,
            ...
            comment='TEST', collection_name='TEST', meta=None, workers=None, qual_id=None,
    ...
    def setQual(self, performance_thresh=90):
        if self.workers == None:
            ...
            self.qual = create_requirement(self.conn, self.workers, self.qual_id, performance_thresh)
    ...
    def give_bonus(self, workers, assignment_ids, bonus_price=0, reason=' '):
        """Give bonus to qualified workers. Workers and assignments are corresponding lists.
        """
        bonus = Price(bonus_price)
        if len(workers)==len(assignment_ids):
            for idx,worker_id in enumerate(workers):
                self.conn.grant_bonus(worker_id, assignment_ids[idx], bonus, reason)
                print worker_id, assignment_ids[idx]
        else:
            print 'Inconsistent workers and assignments'
...
def create_requirement(conn, workers, qual_id, performance_thresh):
    """Returns an MTurk Qualification object for a list of workers which 
    can then be passed to a HIT object. 
        When worker and qual_id exist, this function is called to give qualification.
        Qualitication must be created on webpage (Manage; qualification types) 
    to obtain qual_id.
    """
    # qualification type must already exist, for now.
    #qual_id = '256EY5COW0LLFVI8IO6LL6VNU14W5Z' 
    for worker in workers:
        try:
            conn.assign_qualification(qual_id, worker, value=100, send_notification=True)
        except Exception, e:
